<template>
    <div>
        ...
        <el-row>&nbsp;</el-row>
        <el-table stripe border :data="report" style="width:100%">
...
export default {
    ...
    methods: {
        ...
        async refresh() {
            const response = await this.$axios.get('/report/list');
        ...
        async deleteR(index, row) {
            const response = await this.$axios.get('/report/delete/' + row.id);
        ...
        async onSubmit() {
            const response = await this.$axios.post('/report/' + this.type, this.form);
            if (response.data.code.toString() === '200') {
                this.refresh();
            }
            Message.caseCode(response.data.code);
